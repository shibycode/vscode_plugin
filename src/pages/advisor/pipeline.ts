...
// 导出一个名为 customizeAdvisor 的函数，该函数接收一个 CustomAdvisorConfig 类型的参数 props，并返回一个 Advisor 类型的对象
export const customizeAdvisor = (props: CustomAdvisorConfig): Advisor => {
  // 从 props 中解构出 charts、scopeOfCharts 和 ruleConfig，分别赋值给 CKBCfg 和 ruleCfg
  const { charts, scopeOfCharts: CKBCfg, ruleConfig: ruleCfg } = props;

  // 初始化一个空对象 customCKB，用于存储自定义的图表知识库
  const customCKB: ChartKnowledgeBase = {};
  // 遍历 charts 数组，对每个图表进行处理
